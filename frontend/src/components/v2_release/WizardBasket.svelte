<script lang="ts">
  ...
  <div class="flex-shrink-0 w-1/3 p-4 overflow-y-auto bg-[#f2f3f5]">
    ...
    {#if (($otBookStore && $otBookStore.length > 0) || ($ntBookStore && $ntBookStore.length > 0))}
      {#each $otBookStore as bookCodeAndName}
        <div class="inline-flex items-center justify-between w-full rounded-lg p-6 bg-white
                    text-[#66768B] mt-2">{bookCodeAndName.split(/, (.*)/s)[1]}
    ...
    {#if (($lang0ResourceTypesStore && $lang0ResourceTypesStore.length > 0) || ($lang1ResourceTypesStore && $lang1ResourceTypesStore.length > 0))}
      {#each $lang0ResourceTypesStore as resourceTypeCodeAndName}
        <div class="inline-flex items-center justify-between w-full rounded-lg p-6 bg-white
                    text-[#66768B] mt-2">{resourceTypeCodeAndName.split(/, (.*)/s)[1]}
