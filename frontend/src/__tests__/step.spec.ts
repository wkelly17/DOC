...
test('component renders with inputs', async () => {
  const { getByLabelText, getByPlaceholderText, getByText } = render(FauxStepForm, {
    props: { name: 'test_form' }
  })

  expect(getByLabelText('Test Input').attributes.getNamedItem('name')?.value).toBe(
    'test_input'
  )
  ...
  expect(getByLabelText('Test Input 2').attributes.getNamedItem('name')?.value).toBe(
    'test_input_2'
  )
  expect(() => getByLabelText('Test Input 3')).toThrow()
  ...
  expect(() => getByLabelText('Test Input')).toThrow()
  expect(() => getByLabelText('Test Input 2')).toThrow()
  expect(getByLabelText('Test Input 3').attributes.getNamedItem('name')?.value).toBe(
    'test_input_3'
  )
