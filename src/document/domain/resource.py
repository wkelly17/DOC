...
class USFMResource(Resource):
    ...
    def _initialize_verses_html(self) -> None:
        ...
        chunks, augment HTML output with additional HTML elements.
        """
        parser = bs4.BeautifulSoup(self._html_content, "html.parser")
        ...
        for chapter_break in chapter_breaks:
            ...
            for verse_element in chapter_verse_list:
                (
                    verse_num,
                    verse_content_str,
                ) = USFMResource._verse_num_and_verse_content_str(
                    self.book_number,
                    self.lang_code,
                    chapter_num,
    ...
    def _verse_num_and_verse_content_str(
        ...
        verse_content_str = '<span class="v-num"{}'.format(
...
class TNResource(TResource):
    ...
    def _initialize_verses_html(self) -> None:
        """
        Break apart the TNResource's HTML content into HTML chapter
        and verse chunks, augment HTML output with additional HTML elements
        and store in an instance variable.
        ...
        md: markdown.Markdown = self._markdown_instance(
            self.lang_code,
            self.resource_type,
            self.resource_requests,
        )
        ...
        if not book_intro_path:
            book_intro_path = glob(
                "{}/*{}/front/intro.txt".format(self.resource_dir, self.resource_code)
            )
        book_intro_html = ""
        if book_intro_path:
...
class TQResource(TResource):
    ...
    def _initialize_verses_html(self) -> None:
        ...
        chapter_dirs = sorted(
            glob("{}/**/*{}/*[0-9]*".format(self.resource_dir, self.resource_code))
        )
        # Some languages are organized differently on disk (e.g., depending
        # on if their assets were acquired as a git repo or a zip).
        # We handle this here.
...
class TWResource(TResource):
    ...
    def _initialize_verses_html(self) -> None:
        """
        Break apart the TWResource's HTML content into HTML chapter
        and verse chunks, augment HTML output with additional HTML elements
        and return result.
        """
        # Create the Markdown instance once and have it use our markdown
...
class ResourceProvisioner:
    ...
    def _update_resource_dir(self) -> None:
        ...
        subdirs = [
            file.path
            for file in os.scandir(self._resource.resource_dir)
            if file.is_dir()
        ]
        if subdirs:
